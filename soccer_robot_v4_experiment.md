...
### EXP17 最终判读(完成 2000 iter,部分成功)
稳态 300 窗口(iter~700-1999,避开启动 transient):
| 指标 | EXP16 | EXP17 | 判定 |
|---|---|---|---|
| out_of_bounds | 0.345 | **0.262** | ✅ 降24%,但未到≤0.20目标 |
| 真实射门率 | 0.319 | 0.298 | ≈守住(略降6%,噪声内) |
| fell_over | 0.044 | 0.059 | ✅<0.1护栏 |
| pos_err | 0.91 | 0.95 | ✅护栏 |
| ball_speed_peak | 2.96 | 3.20 | ✅踢力更强 |
| robot_to_ball | 0.91 | 1.12 | ⚠️升高 |
**评估:time_out 语义修复结构正确且有效**——out_of_bounds 降 0.345→0.262 而射门率没塌(0.30,在 EXP16 0.32 噪声内),fell_over/pos_err 护栏守住。**证明根因诊断正确**:去掉 bootstrap 后出界第一次"有代价"。
**但未达 ≤0.20 目标,根因已定位**:`oob_penalty_frac~0.0002`——一次性 OOB 惩罚只在极少步触发,dt 缩放后~0.10 的一次性量级**太弱**,压不过~3.2 峰值踢球把身体往前拉过线的收益。robot_to_ball 升到 1.12(射门率仍守住)= EXP16"大力踢→球飞远→追→过线"机制仍在,只是被减弱。**不是研究红线的"变保守"(射门率没塌)**。
**下一步 EXP18(加强边界信号,研究第3步)**:一次性硬罚太稀疏,改加**密集软边界 shaping + 朝边界速度惩罚**(研究菜单方法1+5):近边界时按深度连续罚 + 罚 root 朝外速度,给每步梯度而非稀疏一击。保留 EXP17 的 time_out=False + 球门走廊(已证有效)。bootstrap EXP17 model_1999。目标 oob≤0.20 而射门率守 0.30。

### EXP18 中止判读(iter~1083,软边界 shaping 权重太弱,诊断完成提前止损)
软边界 shaping(soft_boundary −0.3 + vel_toward_boundary −0.5)叠在 EXP17 之上。稳态:
| 指标 | EXP17 | EXP18 iter~1083 | 判定 |
|---|---|---|---|
| out_of_bounds | 0.262 | 0.26-0.29(各窗口) | ✗ 无改善(与EXP17统计同) |
| 真实射门率 | 0.298 | 0.30-0.32 | ✅守住甚至略升 |
| fell_over | 0.059 | 0.07-0.08 | ✅护栏 |
| pos_err | 0.95 | 0.96 | ✅护栏 |
| robot_to_ball | 1.12 | 1.06-1.23 | ⚠️仍偏高 |
**根因(Episode_Reward 量级直接看出)**:soft_boundary −0.03、vel_toward_boundary −0.018,而 **goal_progress +0.53**(主驱动)——边界惩罚比朝门拉力小 ~20 倍,只能微调不能压过"朝门冲"行为。OOB 因此纹丝不动(甚至随射门变强微升)。**不是方向错,是权重太小**。
**决定**:iter~1083 已确认 OOB 在各窗口稳定 0.26-0.29 = 平台,不浪费剩余 900 iter,止损转 EXP19。
**下一步 EXP19**:把软边界权重大幅提高(soft_boundary −0.3→−2.0,vel_toward_boundary −0.5→−3.0,量级对齐 goal_progress 的 ~0.5),并把软带 soft_margin 1.5→2.0m 提前预警。bootstrap EXP17 model_1999(干净起点,非 EXP18 半截)。判据不变:oob≤0.20 而射门率守 0.30、护栏不破。若射门率塌=过头回退。

### EXP19 最终判读 + OOB 修复线总结(完成 2000 iter)
EXP19 把软边界权重提到 goal_progress 量级(soft_boundary −2.0、vel_toward −3.0、soft_margin 2.0)。稳态(末100):oob 0.294、射门率 0.325、fell_over 0.063、pos_err 0.96、robot_to_ball 1.07。中途 soft_boundary_depth 一度从 0.19 降到 0.13(策略确在学着躲软带),但 OOB 最终回弹到 ~0.29。

**OOB 修复线(EXP17→18→19)总结**:
| 指标 | EXP16 | EXP17 time_out修复 | EXP18 弱软边界 | EXP19 强软边界 |
|---|---|---|---|---|
| out_of_bounds | 0.345 | **0.262** | 0.26-0.29 | 0.29 |
| 真实射门率 | 0.319 | 0.298 | 0.30-0.32 | 0.325 |
| fell_over | 0.044 | 0.059 | 0.07 | 0.063 |
| pos_err | 0.91 | 0.95 | 0.96 | 0.96 |

**核心结论:time_out 语义修复(EXP17)拿到了几乎全部可回收的 OOB 降幅(0.345→0.262),边界 reward shaping(无论弱/强)都压不过 ~0.26 这个结构地板。** EXP19 强权重证明:加大惩罚→射门率没塌(0.325 健康)但 OOB 也不降——不是权重不够,是结构天花板。

**结构根因(研究文档已预警)**:`goal_line_x == half_length == 11.0`,OOB margin 仅 0.3m(限 10.7)。要进球(球过 x=11)机器人本就必须在边界处/外作业。**这是真天花板,不是调参问题。** 球门走廊(|y|<1.3 放宽 0.6m)已缓解正面射门,但侧向逼近/追飞球仍贴边界。

**最佳模型 = EXP17 model_1999**(oob 0.262 最低 + 射门率 0.298 + 护栏最稳),存档为 OOB 修复线产物。EXP18/19 不优于 EXP17,不采用。

**下一步方向(留给后续)**:OOB 已不是 reward 能再压的;真要降需**改场地/任务几何**(如把 goal_line_x 设在 half_length 内侧留缓冲带,或进球判定改"球过线"而非"机器人不出界"的耦合)。但 OOB 0.26 在 FIFA/RoboCup 语境下已是可接受护栏(出界=换边球,非致命),**建议转回主线提升射门率(0.32→0.5)而非继续抠 OOB**。

## EXP16-19 多指标综合评价(2026-06-10,踢球链 + 越界修复线汇总)
统一用稳态末100采样点判读(EXP18 在 iter~1093 止损,取其末100)。

### 全指标对照表
| 维度 | 指标 | 方向 | EXP16 | EXP17 | EXP18 | EXP19 |
|---|---|---|---|---|---|---|
| **踢球链(主)** | 真实射门率(子集) | ↑ | 0.319 | 0.288 | 0.313 | **0.325** |
| | goal_rate(全局) | ↑ | 0.154 | 0.149 | 0.166 | **0.173** |
| | target_is_goal | ↑ | 0.483 | 0.517 | 0.530 | **0.531** |
| | episode_success | ↑ | **0.479** | 0.434 | 0.409 | 0.449 |
| | possession | ↑ | **478.7** | 454.4 | 413.1 | 454.2 |
| | ball_speed_peak | ↑ | 2.957 | **3.203** | 3.291 | 3.181 |
| | ball_to_tgt_err | ↓ | **2.347** | 2.788 | 2.796 | 2.765 |
| | ball_stuck_s | ↓ | **0.825** | 1.342 | 1.463 | 1.335 |
| | robot_to_ball | ↓ | **0.914** | 1.162 | 1.228 | 1.069 |
| **稳定性** | fell_over | ↓ | **0.044** | 0.048 | 0.087 | 0.063 |
| | out_of_bounds | ↓ | 0.345 | **0.256** | 0.292 | 0.294 |
| **自定位** | pos_err_m | ↓ | **0.913** | 0.927 | 0.964 | 0.964 |
| | scan_uniq_frac | ↑ | **0.172** | 0.159 | 0.158 | 0.157 |

### 加权综合评分(min-max 归一 × 任务优先级权重)
权重:射门率0.30 / goal_rate0.12 / fell_over0.13 / out_of_bounds0.12 / episode_success0.10 / pos_err0.08 / 其余各0.05。
| 实验 | 综合分 | 排名 | 一句话 |
|---|---|---|---|
| EXP16 | 0.686 | 1(微) | 次要指标(success/possession/robot_to_ball/b2t)最好,但 OOB 0.345 最差 |
| **EXP19** | **0.681** | 2(微) | **主目标最强**(射门率/goal_rate/tgt_is_goal 三冠)且 OOB 0.294<EXP16 |
| EXP18 | 0.409 | 3 | 弱软边界,夹在中间无亮点,fell_over 0.087 偏高 |
| EXP17 | 0.380 | 4 | OOB 0.256 最低(唯一亮点),但射门率 0.288 垫底 |

### 结论与代表性实验选择
- **综合分 EXP16(0.686)≈ EXP19(0.681)并列**,但两者赢的维度不同:EXP16 赢在次要指标 + robot_to_ball,EXP19 赢在**核心主目标(射门率+进球)**且**修好了 EXP16 最大的痛点 OOB**(0.345→0.294)。
- **代表性实验 = EXP19**:在"进球 + 不出界"两个核心维度上 Pareto 优于 EXP16(射门率 0.325>0.319 且 OOB 0.294<0.345),完整跑满 2000 iter,继承全部修复(time_out + 球门走廊 + 软边界 shaping)。综合分仅差 EXP16 0.005 在噪声内,而主目标领先是实打实的。
- EXP17 是越界修复线的**机制验证最佳**(OOB 0.256,证明 time_out 修复是胜负手),但作为部署模型射门率垫底,不选。EXP18 全面被 EXP19 支配,不选。
- **代表权重已存** → `checkpoints/v4_soccer/` 与 `soccer_eval/2026-06-09_v4/`(均为 EXP19 model_1999)。

## POV/深度可视化代码审查(2026-06-10,用户质疑触发的深挖)
用户质疑:POV 视频里看不到球、球如何追踪;depth POV 图看不出价值。逐一代码级验证(含纠正自己探针的 sim.forward 不渲染 bug),两点都查实:

### 发现1:相机几何盲区(真实约束,非bug)
- 实测头部相机:**高 0.79m、俯角≈0°(平视,不低头)**,fovy 60°(垂直半视场30°)。球在地面 z=0.07。
- 球进入视野条件 `atan((0.79-0.07)/d) < 30°` → **d > ~1.25m 才看得到球**。
  - 球 1.25–3m(接近阶段):相机看得到(dist=1.54m 正对时放球差分确认红球清晰出现在画面左下)。
  - 球 <1.25m(带球/踢球阶段):落入相机下方盲区,RGB+深度都看不到球。即 `mos92_foot_zone_blind_spot`。
- **结论**:触球时策略不靠当前帧看球,而是①接近时(1–3m)看到球定位 ②之后靠 EKF 自定位 + RGB 6帧时序记忆推算球相对位置。设计约束,非bug。但旧 README "相机看到球→推球方向"过简,触球阶段实际看不到球,需修正措辞。
- 关键探针陷阱:**渲染只在 sim.step() 的 _sense_kernel 触发,sim.forward() 不渲染**;用 forward 读 sensor.data 得到旧缓存,会误判"球0像素"。必须用真实 env.step() 测。

### 发现2:depth POV 可视化bug(已修)
- 视频 depth 面板喂的是**原始传感器深度**(0~1000m 到地平线),imshow 自动按 max 归一化 → 近场 0–3m 全压成近黑,看着没信息。
- 但**策略实际吃的是 `camera_depth`:裁剪到 3m 再归一化**(manipulation/mdp/observations.py:104),近场对比度高、能分辨球门/近处球。可视化未还原策略真实输入,是可视化层 bug。
- 修复:depth 面板改用 clip(0.05,3.0)/3.0(与策略一致),并在画面标注"球在视野内/盲区"。

## EXP20 设计方向 + 仿真持续学习讨论(2026-06-10,用户提出)

### 发现:机器人能低头,但当前策略几乎没用
- `neck_pitch` 是**策略可控的动作关节**(在 joint_pos action 的 20 维里),且**无限位**(jnt_limited=0)。
- 但实测当前 EXP16 策略只把 neck_pitch 俯到 **~−11°**(cam_pitch −11.6°~+1.8°),远不足以低头看 0.5m 处的球。
- 后果:相机平视(俯角≈0°),地面球仅在 >~1.25m 进入视野;带球/踢球(球<1.25m)球在脚下盲区。
  策略靠 EKF 自定位 + RGB 6 帧时序记忆维持对球的相对判断,而非当前帧看球。

### EXP20 候选方向(让机器人学会低头看球)
1. **奖励引导低头**:当球在近距离(<1.5m)且在身前时,奖励 neck_pitch 朝下(让球进入视野下半部)。
   风险:可能干扰自定位(低头看不到远处地标→coverage 掉→pos_err 升)。需平衡"看球"vs"看地标"。
2. **gaze-at-ball 课程**:复用已有 gaze 奖励族(gaze_center/gaze_search),把目标从"球门朝向"扩展到
   "近球时低头注视球",远球时抬头扫地标。即**主动视觉的注意力切换**。
3. **相机硬件改造(治本)**:把头部相机默认俯角调下(改 `_HEAD_CAM_QUAT`)或加宽 fovy,缩小脚下盲区。
   代价:改了相机要重训感知 CNN;且过度低头会丢失远处地标(自定位变难)。是几何 trade-off。
4. **解耦观测**:再加一个**专门朝下的脚部相机**看近球(类似真实人形的胸/腰相机),
   头部相机专注远处地标自定位。代价:多一路 CNN、算力↑。

> EXP20 红线:任何"低头看球"的改动都要守住自定位护栏(pos_err<1.1)和踢球能力(射门率不塌)。
> 优先级:先试方向 1/2(纯奖励,不改硬件、可快速验证),不行再考虑 3/4(改硬件/加传感器,重训成本高)。

### 仿真持续学习(continuous learning)讨论
**问题**:如何让机器人在仿真中持续产/学?后期是否需要?

**现状**:当前是**离线分阶段训练**(EXP1→19,每轮固定环境跑满 2000 iter 后判读、改、再训),
不是持续学习。每轮都从上一轮 bootstrap,但环境/奖励是人工调的、训练是有限步的。

**几种"持续学习"的含义与适用性**:
1. **课程式持续训练(最现实,后期需要)**:不在固定难度停,而是随能力自动升难度
   (出生几何从全场→进攻半场、球门越来越难、加干扰/噪声)。已部分在用(gt_mask/penalty_ramp/spawn_geometry
   课程),后期应**自动化课程**(按 success_rate 触发难度提升),避免人工逐轮调。这是最该上的。
2. **域随机化持续化(后期需要,为 sim-to-real)**:持续随机化质量/摩擦/相机噪声/延迟,让策略对分布鲁棒。
   当前 penalty_ramp 之外随机化有限;真要上实机前必须加。
3. **在线/终身学习(部署后不断更新权重)**:机器人一边踢一边更新策略。**当前阶段不需要**——
   ① 仿真里没有"新分布持续涌入"的必要(我们能直接造任意场景);② 在线 RL 不稳、易遗忘;
   ③ 部署阶段更适合"冻结策略 + 离线再训练迭代",codex 的血泪也指向端到端离线训练而非在线。
4. **自动化实验循环(工程意义的"持续")**:让训练→判读→改→再训的循环自动跑(我现在手动做的事脚本化)。
   后期值得做以提速,但属于 MLOps 不属于算法。
...
### EXP20 最终判读(完成 2000 iter,全面正向,射门率 0.32→0.34)
稳态末100(末200括注):
| 指标 | EXP19 | EXP20 | 判定 |
|---|---|---|---|
| 真实射门率 | 0.325 | **0.340**(末200 0.346) | ✅ 升,未到 0.40 目标 |
| pos_err | 0.964 | **0.851** | ✅✅ **历史最好**(低头没伤定位,反而修对几何后更好) |
| out_of_bounds | 0.294 | **0.212** | ✅✅ **突破 0.26"结构地板"**(EXP18/19 shaping 压不动的,瞄准变好后自己下来了——侧证出界确是踢歪的副产品) |
| ball_to_tgt_err | 2.765 | **2.159** | ✅✅ 历史最好(踢得更靠近门) |
| robot_to_ball | 1.069 | **0.724** | ✅✅ 踢飞-追球循环明显缓解 |
| fell_over | 0.063 | **0.038** | ✅ 历史最稳 |
| episode_success | 0.449 | 0.530 | ✅ |
| ball_speed_peak | 3.181 | 2.891 | 🟡 略降但守住>2.8 护栏(没掉进贴球吸引子) |
| kick_lat_align | — | 0.40→0.49 | ✅ 对齐行为确实在学 |

**评估:三件套(gaze pitch 符号修复 + neck_pitch 放开 + 横向对齐)方向正确**——"踢不准"的下游症状
全线缓解(ball_to_tgt↓、robot_to_ball↓、OOB↓),且两个此前认为"结构性"的指标(OOB 地板、pos_err
0.9 平台)都被打破,**证明它们其实都是"踢歪"的副作用**。射门率本身 0.34(+5%/EXP19,+7%/EXP16)
进步但未达 0.40:末段趋势(末400 0.324→末200 0.346)仍在缓慢上行,未完全收敛。

**下一步**:① 先续训 2000 iter(EXP20b,同配置从 EXP20 model_1999 续,验证是否还有上行空间)
② 若续训也平台在 ~0.35,则下一张牌是方法论清单上的"踢腿参考轨迹"(arXiv:2407.14612)或
"球相对脚 obs+历史帧"(预测接触时刻蓄力),并考虑跑 codex 式 GT-command 上界探针校准 0.5-0.6
目标在我们 episode 时长下的物理可达性。

### EXP20b 最终判读(续训完成,射门率 0.34→0.38-0.41,持续上行)
稳态读数(多窗口):末400 **0.387**、末200 0.381、末100 0.368、末50 **0.410**;中段一度 0.416@1424。
| 指标 | EXP20 | EXP20b | 判定 |
|---|---|---|---|
| 真实射门率 | 0.340 | **0.38-0.41**(波动中枢~0.39) | ✅ **突破 0.40 线一只脚**,EXP16 的 0.32→+20% |
| ball_to_tgt_err | 2.159 | **1.881** | ✅✅ 历史最好(球离门 <2m) |
| robot_to_ball | 0.724 | **0.599** | ✅✅ 回到基线水平(踢飞-追球循环基本消除) |
| episode_success | 0.530 | **0.581** | ✅ 历史最好 |
| out_of_bounds | 0.212 | **0.202** | ✅ 维持地板下方 |
| pos_err | 0.851 | 0.882 | ✅ 护栏内(0.83-0.93 波动) |
| fell_over | 0.038 | 0.063 | ✅ <0.1 |
| ball_speed_peak | 2.891 | 2.700 | 🟡 缓降但守住护栏(>2.7),需继续盯 |

**结论:EXP20 三件套是 v4 开始以来单次增益最大的一手**(射门率 0.32→0.39,+20%;5 项历史最好)。
"修对视觉几何让机器人能看准球,再要求摆正身体踢"——瞄准链打通后,出界/追飞球/落点散这些
此前各自为战的症状一起好转。0.5-0.6 终极目标还差 ~0.1-0.2,且 ball_speed_peak 缓降值得警惕
(对齐奖励可能在轻微 trade 踢力,贴球吸引子风险未消)。

**交接备注(给下一轮)**:① 最佳模型 = EXP20b model_1999
(logs/.../2026-06-10_20-22-43_spike_v4_e2e_ekf_kick_aim2);② 若续训第三轮,先查 peak 是否继续降——
若降破 2.5 应停,转"踢腿参考轨迹"(方法论清单)或提高 kick_impulse 权重对冲;③ codex 式 GT-command
上界探针(校准 0.5-0.6 的物理可达性)仍值得跑;④ EXP20 系沿用 oracle 球检测,真检测器阶段(B1)未动。

## EXP20b 射门失败解剖(2026-06-10,12h自主推进开篇:冷静分析)
新诊断工具 scripts/diag_v4_shot_forensics.py:64envs x 3000steps,193个球门episode逐个记录败因
(deep_x/y@deep/spawn距离/每次kick的位置+瞄准角误差+球速)。结果:

| 败因 | 占比 | 解读 |
|---|---|---|
| SCORED | 31.6% | — |
| **SHORT** | **44.6%** | **球到x=10.5(中位,距门线0.5m!)却没推过线;中位踢9脚、用满1000步** |
| NEVER_ARRIVED | 17.6% | spawn距门中位12m,20s物理到不了(时间预算,非能力) |
| WIDE | 6.2% | 踢偏出底线——瞄准已非主要问题 |

**分区kick数据(1817次踢球接触)**:
- 近门区(x>7): n=809,kick球速中位 **0.47 m/s**,瞄准误差中位 24.5°
- 中场(0-7): n=926,kick球速中位 **0.69 m/s**,瞄准误差中位 16.2°
- **越近门踢得越轻**——近门退化成小碎步轻推,缺"临门一脚"的发力转换(对应人类足球的射门姿态切换)。

**三个结论**:
1. 主攻方向=**近门终结力**(SHORT 44.6%是最大败因):球已在门口0.5m,差的是把球果断推/射过线的那一下。
2. NEVER_ARRIVED 17.6%是episode时间预算上限(spawn 12m),**射门率可达上限≈0.82**;这部分不是能力缺陷,
   评估时应当分开统计(新指标需求:按spawn距离分桶的射门率)。
...
### EXP21 最终判读(完成2000iter,机制达标但总量持平,瓶颈转移)
终态(末100/末300): 射门率 0.364/0.384(中途末100一度0.421,波动带0.36-0.42)≈ EXP20b 的 0.368;
oob **0.185-0.21**(更优)、fell 0.071、pos_err 0.91、peak 2.81(守住)。

**Forensics 对照(各193个球门episode,同法)**:
| 败因 | EXP20b | EXP21 | 判定 |
|---|---|---|---|
| SCORED | 31.6% | 34.2% | ✅ +8% |
| **SHORT** | **44.6%** | **23.3%** | ✅ **腰斩,机制修复达标(判据<30%)** |
| NEVER_ARRIVED | 17.6% | **35.2%** | ❌ 翻倍(瓶颈转移) |
| WIDE | 6.2% | 7.3% | ≈ |
近门kick速度: 中位0.47→0.52、均值→0.64(判据>0.6 部分达标);中场kick速度 0.69→0.60(微降)。

**评估**:
- ✅ **机制假设被证实**: 走廊x豁免后,"到门口停住"问题腰斩(44.6%→23.3%),近门踢球速度上升——
  数值算账→单变量修复→定向改善,这条因果链完整闭环。oob 还顺带又降(0.202→0.185)。
- ❌ **总射门率持平**: SHORT省下的episode大半流入NEVER_ARRIVED(17.6%→35.2%)而非SCORED。
  NEVER_ARRIVED spawn中位10.9m,且中场kick速度微降(0.69→0.60)——**新瓶颈=到达速度/每球推进效率**:
  20s预算内从10m+把球推进门需要更少、更有效的触球(EXP21中位每episode仍9+次kick)。
- 结论: 终结通了,现在缺的是**触球质量**(每脚踢得更远更准→更少脚数→时间预算内到门)。
...
### EXP22b 最终判读(完成2000iter)⚠️ 本节 forensics 数字作废,见下方「EXP22b forensics 测量错误更正」
预接触窗口版 plant(信号增强~80x)。终态: 射门率末300 **0.395**/末100 0.416(中途稳定在0.42-0.43带,
**v4首次站上0.40**);pos_err 0.886(新高)、robot_to_ball 0.59-0.65(新高)、ball_to_tgt 1.78-1.83(新高)、
fell 0.059、oob 0.20-0.22(守住)。ball_speed_peak 2.62-2.69(缓降但>2.5护栏)。

**Forensics 对照(192 episodes)**:
| | EXP20b | EXP21 | EXP22b | 判定 |
|---|---|---|---|---|
| SCORED | 31.6% | 34.2% | **37.0%** | ✅ 持续升 |
| SHORT | 44.6% | 23.3% | 40.6% | ⚠️ 回升(见下) |
| NEVER_ARRIVED | 17.6% | 35.2% | **16.7%** | ✅ **中场球速0.73(达标≥0.75边缘)解决了到达问题** |
| 中场kick速度(中位) | 0.69 | 0.60 | **0.73** | ✅ 判据达标 |
| 近门kick速度(中位) | 0.47 | 0.52 | 0.50 | ⚠️ 仍弱 |
| kicks/ep(中位) | 9 | 9 | 9 | ✗ 未降(判据≤6未达) |

**评估**: plant塑形把中场推进修好了(NEVER_ARRIVED 35%→17%,中场球速+22%),SCORED 三轮连升
31.6→34.2→37.0;但SHORT回升40.6%——**到达变快了,门口的轻推问题就重新成为主导**(EXP21修的是
门口惩罚结构,EXP22b没修门口发力)。近门球速三轮卡0.47-0.52,kicks/ep卡9:**"临门一脚发力"是
当前真正顽固的瓶颈**,它不是惩罚结构问题(EXP21已豁免)而是技能问题——近门小碎步轻推是策略学到的
局部最优(每次轻推都有goal_progress正反馈,大力射门有摔倒/出界风险)。

**12h窗口决定**: EXP22b作为窗口最终模型(射门率0.40-0.42带 vs 起点EXP20b 0.368,+8-13%;
五项次要指标全新高)。"近门发力"杠杆留给下一窗口:候选方案=近门区kick_impulse权重梯度
(x>9时加倍)或shot_clearance奖励(球离脚瞬时速度>1.5才计progress),需先离线验证不破坏EXP21成果。

## 12h 自主推进窗口总结(2026-06-10 21:00 → 06-11)⚠️ 表中 forensics 行(SCORED/NEVER_ARRIVED/球速)已被审计更正,以「测量错误更正」节为准

**主线成果(压榨当前框架)**:
| | 窗口起点(EXP20b) | 窗口终点(EXP22b) |
|---|---|---|
| 真实射门率 | 0.368 | **0.395-0.416(0.42-0.43带出现)** |
| SCORED(forensics) | 31.6% | **37.0%** |
| NEVER_ARRIVED | 17.6% | 16.7%(中场球速 0.69→0.73) |
| pos_err / robot_to_ball / ball_to_tgt | 0.88/0.60/1.88 | 0.89/0.59/1.78(均守住或新高) |
| fell / oob | 0.063/0.202 | 0.059/0.20-0.22 |

**方法论沉淀(比单点数字更重要)**:
1. **射门解剖工具**(diag_v4_shot_forensics.py + §7 方法学): 四分类败因+分区kick质量,
   三轮实验全靠它定位真瓶颈,替代"瞪聚合指标猜原因"。
2. **完整因果链三连**: 数值算账(走廊净亏损-0.036)→单变量修复(EXP21豁免)→定向改善(SHORT腰斩);
   信号量级诊断(plant 0.0003≈0)→门控重设计(EXP22b预接触窗口)→中场球速达标。两次都先诊断后动手。
3. **止损纪律执行两次**(EXP22接触帧版、EXP18同型),各省~1000 iter。
4. **新训练指标**: ball_in_finish_box/ball_deepest_x/support_plant 系列接入,训练日志可直读漏斗。
5. **codex情报转化**: pre_contact_window(6.03x)直接救活了 plant 杠杆;7.86x support-foot 验证为有效方向。

**交接给下一窗口**:
- 主攻: **近门发力**(近门球速卡0.5、kicks/ep卡9、SHORT回升40.6%)——技能局部最优,非惩罚结构。
  候选: 近门kick_impulse权重梯度 / shot_clearance(球离脚速度>1.5才计progress)。先离线验证。
- 盯防: ball_speed_peak 三轮缓降(2.82→2.70→2.65),仍>2.5但趋势不健康(codex"低速贴球吸引子"前兆);
  若下轮跌破2.5需回查 plant/lat_align 是否过度抑制摆腿。
- 路线A(真纯视觉): 调研已完成存档 v4真纯视觉路线调研.md(三阶段: odometry加噪→真实关键点CNN→
