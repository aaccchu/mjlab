...
## 十一、目标重对齐 + 路线修正(2026-06-09,EXP10 后 + 借鉴 codex)

### 最终目标(权威,不变)— 三者必须同时达成
1. **纯视觉自定位 pos_err 接近 0.79m**(01 水平,从 5.8m 提升)+ 距离自适应精度。
2. **踢球能力 dribble_success > 0.4**(对标 04_e2e 的 0.51)。
3. **进球 goal_rate > 0.2**(对标 04 的 0.30),**fell_over ≈ 0**。
难点在"同时"——单独优化任一项都会挤掉其他(codex C8 射门退化、我 EXP10 neck 甩头破稳,同源张力)。

### 当前位置(EXP6-10 后)
- 自定位 5.8m → **1.53m**(进展大,未达 0.79m)；fell_over ≈ 0.07(达标)。
- **goal_rate 0.075、踢球链疑似断裂**(❌ 离 0.2 远）。

### 路线修正(借鉴 codex EXP7C8-C21 的决定性教训)
**真瓶颈不止是自定位,更是踢球链**:codex 用 GT 作弊在近脚窗口注入推球命令,goal 仍仅 0.018(上界 0.68)
→ 冻结的 04_e2e actor 无法把"看见球"转成"推球进门"。**"会看见≠会踢"**。
→ v4 剩余工作拆成两条独立线:
  - **线A 自定位**(进行中):修融合 bug → 重跑 → 回答"健康融合能否把 pos_err 推近 0.79m"。
    这是廉价高信息量步骤,不碰踢球链。
  - **线B 踢球链**(下一阶段):接 codex 诊断,训练/蒸馏 belief→近脚窗口→contact primitive,
    目标恢复 dribble_success>0.4、goal_rate>0.2。这才是 goal_rate 上不去的根因。
