...
from mjlab.tasks.velocity.config.mos92.env_cfgs import (
  mos92_soccer_e2e_dualcam_keypoint_env_cfg,
)
...
def main() -> None:
  ...
  env_cfg = mos92_soccer_e2e_dualcam_keypoint_env_cfg(play=False)
  ...
  agent_cfg.run_name = "spike_v4_e2e_keypoint"
  agent_cfg.save_interval = 100

  # EXP5f A': supervised keypoint aux loss via local KeypointAuxPPO subclass.
  # The label group must be declared in obs_groups so resolve_obs_groups keeps it
  # in the rollout storage; it is NOT in actor/critic sets, so the policy never
  # sees it — only the aux loss consumes it. class_name points at the in-project
  # subclass (resolved by dotted path), leaving vendored rsl_rl untouched.
  agent_cfg.obs_groups["keypoint_label"] = ("keypoint_label",)
  agent_cfg.algorithm.class_name = "mjlab.rl.keypoint_aux_ppo:KeypointAuxPPO"
  agent_cfg.algorithm.keypoint_cfg = {
    "action_start": 20,
    "action_end": 66,
    "label_group": "keypoint_label",
    "coef": 1.0,
    "num_keypoints": 23,
  }

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_keypoint")
