...
from mjlab.tasks.velocity.config.mos92.env_cfgs import (
  mos92_soccer_e2e_dualcam_env_cfg,
)
...
BASE_CKPT = Path("checkpoints/v3_soccer_solo/04_e2e_integrated/model_1499.pt")
NUM_ENVS = 384
...
def main() -> None:
  ...
  env_cfg = mos92_soccer_e2e_dualcam_env_cfg(play=False)
  ...
  agent_cfg.run_name = "spike_v4_e2e_dualcam"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_dualcam")
