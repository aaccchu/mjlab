"""v4 EXP18: DENSE SOFT-BOUNDARY SHAPING (on top of EXP17's out-of-bounds fix).
...
target. Diagnosis from the logs: oob_penalty_frac ~2e-4 — the one-shot terminal
penalty fires on almost no steps, so its dt-scaled ~0.10 magnitude is far too
sparse to outweigh the ~3.2 peak-kick payoff that pulls the robot goalward over
the line (robot_to_ball rose to 1.12 chasing fast balls off the field).
...
edge so the cost is felt every step, long before the line is crossed:
  - soft_boundary_penalty (weight -0.3): depth (m) into a 1.5 m soft band inside
    each hard line.
  - velocity_toward_boundary_penalty (weight -0.5): outward root speed, gated to
    the soft band — directly damps over-run after a hard kick, never penalizes
    mid-field chasing.
Both honor the goal-mouth corridor so legitimate goalward shots are exempt.

Bootstrap: EXP17 model_1999 (same obs layout, no std reset). Target:
out_of_bounds <= 0.20 WHILE real shot rate holds ~0.30 and guardrails
...
Usage: MUJOCO_GL=egl uv run python scripts/spike_v4_e2e_ekf_kick_oob_soft.py [--smoke]
"""
...
from mjlab.tasks.velocity.config.mos92.env_cfgs import (
  mos92_soccer_e2e_dualcam_ekf_kick_oob_soft_env_cfg,
)
...
# the full kick chain + the boundary-aware policy. EXP18 only adds dense soft
...
def _partial_load_shape_match(module, ckpt_sd) -> tuple[int, int, list[str]]:
  """Load every name+shape-matching tensor; reinit the rest. For EXP18 the obs
...
def main() -> None:
  ...
  env_cfg = mos92_soccer_e2e_dualcam_ekf_kick_oob_soft_env_cfg(play=False)
  ...
  agent_cfg.run_name = "spike_v4_e2e_ekf_kick_oob_soft"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_ekf_kick_oob_soft")
  ...
  if BASE_CKPT.exists():
    ...
    # NO std reset: EXP17 has a well-tuned action std; EXP18 only adds dense
  ...
  print(
    f"[INFO] v4 EXP18 soft-boundary [{mode}]: {max_iter} iters, "
