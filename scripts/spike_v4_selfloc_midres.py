...
from mjlab.tasks.velocity.config.mos92.env_cfgs import (
  mos92_soccer_selfloc_dualcam_env_cfg,
)
...
def main() -> None:
  ...
  env_cfg = mos92_soccer_selfloc_dualcam_env_cfg(play=False)
  env_cfg.scene.num_envs = 32 if smoke else NUM_ENVS

  # EXP5a: bump the RGB self-loc cam 96x72 -> 160x120 (2.78x pixels). The EXP1
  # pure-selfloc dualcam run COLLAPSED after GT withdrawal even without e2e
  # multi-objective (best 1.88m@1836 -> 6.73m@3799), so 96x72 signal is too weak
  # for the RGB CNN to learn visual loc that SURVIVES the crutch pull. The
  # high-res view images proved 0.125m lines are resolvable at higher res. This
  # isolates RESOLUTION as the variable: pure selfloc task, higher RGB res, does
  # selfloc hold after GT goes? depth cam stays 64x48 (untouched).
  for _s in env_cfg.scene.sensors or ():
    if getattr(_s, "name", "") == "head_cam_rgb":
      _s.width = 384
      _s.height = 288
  ...
  # GT-pose fade: in smoke, ramp instantly so we exercise the masked path; in
  # full, keep the env's slower [800, 2500]-iter ramp.
  ...
  agent_cfg.run_name = "spike_v4_selfloc_midres"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_selfloc_midres")
