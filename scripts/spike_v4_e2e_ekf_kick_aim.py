"""v4 EXP20: AIM THE KICK (fix look-down geometry + square up to the ball).
...
Usage: MUJOCO_GL=egl uv run python scripts/spike_v4_e2e_ekf_kick_aim.py [--smoke]
"""
...
def main() -> None:
  ...
  agent_cfg.run_name = "spike_v4_e2e_ekf_kick_aim"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_ekf_kick_aim")
  ...
  if BASE_CKPT.exists():
    print(f"[INFO] Partial-load from EXP19: {BASE_CKPT}")
  ...
  print(
    f"[INFO] v4 EXP20 aim-the-kick [{mode}]: {max_iter} iters, "
