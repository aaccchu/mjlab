...
soccer_eval/2026-06-11_v4_showdown/kick_strike_exp23/ via _v4_scenario_video.py:
  - kick_strike_exp23_approach.mp4 : robot walking up to the ball
  - kick_strike_exp23_strike.mp4   : the kick (peak ball speed)
  - kick_strike_exp23_goalward.mp4 : ball driven toward the goal mouth (scored flag
                                     in scenarios.json if a real goal was captured)
plus one mid-clip still per scenario and scenarios.json.

Usage: MUJOCO_GL=egl uv run python scripts/eval_v4_kick_strike_exp23.py
"""
...
def main() -> None:
  info = run(mos92_soccer_e2e_dualcam_ekf_kick_strike_env_cfg, CKPT, OUT_DIR, PREFIX)
