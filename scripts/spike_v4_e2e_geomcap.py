...
def main() -> None:
  ...
  # EXP3: raise selfloc_accuracy weight 0.8 -> 2.5. EXP2 diagnosis: in the e2e
  # task selfloc(0.8) competes with ~16.5 of dribble/goal reward weight, starving
  # the localization gradient (v3 full-field got 2.80m at 0.8 with NO goal layer;
  # v4 full-field only 6.47m). v3 exp8 same lesson (0.3->0.8 to beat scatter);
  # the e2e task has more competing objectives, so push higher.
  env_cfg.rewards["selfloc_accuracy"].weight = 2.5

  # EXP4: cap the geometry morph at alpha_max=0.6 (don't collapse to pure
  # attacking half). EXP3 showed selfloc is 2.13m on full-field but 5-8m once the
  # spawn tightens to the attacking half (geometry's physical limit). Keeping 40%
  # full-field diversity should hold selfloc low while still giving enough
  # near-goal episodes to score.
  if "spawn_geometry" in env_cfg.curriculum:
    env_cfg.curriculum["spawn_geometry"].params["alpha_max"] = 0.6
  ...
  agent_cfg.run_name = "spike_v4_e2e_geomcap"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_geomcap")
