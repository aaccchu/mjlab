...
Unlike eval_v4_kick_strike_amp_exp24.py (which segments the rollout into three short
...
flashes for ~0.5 s on each scored frame.
  ...
  - kick_strike_amp_exp24_long.mp4   : the full reel
  - long.json                    : rollout stats (steps, goals, fps, duration)

Length knobs (env vars):
  EXP24_LONG_STEPS  total env steps to roll (default 9000 = 180 s @ 50 fps)
  EXP24_LONG_FPS    playback fps (default 50, matches the 50 Hz control rate)

Usage:
  MUJOCO_GL=egl EXP24_LONG_STEPS=12000 uv run python \
    scripts/eval_v4_kick_strike_amp_exp24_long.py
"""
...
STEPS = int(os.environ.get("EXP24_LONG_STEPS", "9000"))
FPS = int(os.environ.get("EXP24_LONG_FPS", "50"))
...
def main() -> None:
  ...
  print(
    f"[INFO] rolling {STEPS} steps ({STEPS / FPS:.0f}s @ {FPS}fps) from {CKPT.name}"
  )
  ...
  print(
    f"[INFO] wrote long reel -> {out_path} ({result['duration_s']}s, {goals} goals)"
  )
