...
Usage: MUJOCO_GL=egl uv run python scripts/spike_v4_e2e_ekf_kick.py [--smoke]
"""
...
# Bootstrap from EXP13 (EKF run, 88-dim same structure): inherits gait + approach +
# EKF localization (pos_err 0.98m). B2 adds the near-foot kick skill on top.
BASE_CKPT = Path(
  "logs/rsl_rl/mos92_velocity/2026-06-09_16-08-16_spike_v4_e2e_ekf/model_1999.pt"
)
NUM_ENVS = 384
MAX_ITER = 2000
...
def main() -> None:
  ...
  agent_cfg.run_name = "spike_v4_e2e_ekf_kick"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_ekf_kick")
