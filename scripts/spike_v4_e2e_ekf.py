...
Usage: MUJOCO_GL=egl uv run python scripts/spike_v4_e2e_ekf.py [--smoke]
"""
...
from mjlab.tasks.velocity.config.mos92.env_cfgs import (
  mos92_soccer_e2e_dualcam_ekf_env_cfg,
)
...
# Bootstrap from EXP11 (fused_fixed, 88-dim same structure): inherits gait+kick+
# belief-consumption. EkfPoseBelief output is 7-dim same layout, so weights load fully.
BASE_CKPT = Path(
  "logs/rsl_rl/mos92_velocity/2026-06-09_15-03-04_spike_v4_e2e_fused_fixed/model_1999.pt"
)
NUM_ENVS = 384
MAX_ITER = 2000
...
def main() -> None:
  ...
  env_cfg = mos92_soccer_e2e_dualcam_ekf_env_cfg(play=False)
  ...
  agent_cfg.run_name = "spike_v4_e2e_ekf"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_ekf")
