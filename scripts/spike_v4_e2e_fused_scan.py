...
Usage: MUJOCO_GL=egl uv run python scripts/spike_v4_e2e_fused_scan.py [--smoke]
"""
...
def main() -> None:
  ...
  agent_cfg.run_name = "spike_v4_e2e_fused_scan"
  agent_cfg.save_interval = 100

  log_root = Path("logs/rsl_rl") / agent_cfg.experiment_name
  stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_spike_v4_e2e_fused_scan")
