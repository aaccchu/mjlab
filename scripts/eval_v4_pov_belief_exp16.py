"""EXP16 — robot-POV + self-localization belief video (the localization story).

Loads the EXP16 baseline checkpoint and writes a 3-panel composite video
(robot camera POV | top-down GT-vs-belief field map | loc-error & coverage
curves) over one episode, into soccer_eval/2026-06-09_v4/kick_exp16/:
  - kick_exp16_pov_belief.mp4
  - pov_belief.json   (episode stats: scored, final/mean loc err, coverage)

Usage: MUJOCO_GL=egl uv run python scripts/eval_v4_pov_belief_exp16.py
"""
...
def main() -> None:
  info = run(
    ...
    OUT_DIR,
    "kick_exp16",
    "EXP16 (baseline): robot POV + self-localization belief",
  )
