...
## 05_selfloc_realspec_0125m — 真场线(0.125m)+ 主动扫视 + 时序记忆
- `model_3400.pt` — 来自 `spike_v3g_selfloc_realspec`。**用诚实的 0.125m 真场线宽**(去掉 01 模型
  依赖的 1.0m 加粗线 sim2real gap),改用真机 RoboCup 解法:主动扫视(neck_yaw 在策略动作里)+
  时序记忆(RGB 6帧×stride6,跨~0.7s 转头扫视窗)+ 相机 96×72。
- **指标(纯视觉 play=False, mask=0):** selfloc_pos_err_m **~5.8m**。训练渐隐期一度到 2.0m,
  GT 全抽到 0 后稳定 4-6m。
- **诚实结论(关键权衡):** 改回 0.125m 真场线后,即便上主动扫视+时序,纯视觉定位也只到 ~5.8m,
  **回不到 01 模型(1.0m 加粗线)的 0.79m**。印证 gate 物理事实:三个相机杠杆(分辨率 64→512²、
  视场角、俯角+窄FOV)单帧下最差位姿可见度均 <1% —— 0.125m 线在低视角相机下,部分位姿
  (角落/背对球门)物理上看不到足够地标。时序积分**有显著帮助**(从单帧">10m/看不见"拉到 ~5.8m),
  但不足以恢复亚米级。
- **两模型取舍:** `01`(1.0m 加粗线, 0.79m)精度高但有 sim2real gap;`05`(0.125m 真场线, 5.8m)
  无 gap、真场可直接用但精度低一个量级。
- **后续路线:** 更高分辨率+更长扫视窗、加显式"扫视找地标"奖励、或折中用 0.25-0.3m 线宽。
