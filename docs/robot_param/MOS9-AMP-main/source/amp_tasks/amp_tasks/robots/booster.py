...
ROBOTLIB_ASSETLIB_DIR = str(Path(__file__).resolve().parents[4] / "data" / "assets")
...
K1_ACTION_SCALE = {}
for actuator in BOOSTER_K1_CFG.actuators.values():
  ...
  for name in names:
    if name in effort and name in stiffness and stiffness[name]:
      K1_ACTION_SCALE[name] = 0.25 * effort[name] / stiffness[name]
