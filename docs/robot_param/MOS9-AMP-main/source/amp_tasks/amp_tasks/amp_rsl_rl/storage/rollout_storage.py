...
from amp_tasks.amp_rsl_rl.utils.trajectory import split_and_pad_trajectories
...
class RolloutStorage:
  ...
  def _save_hidden_states(self, hidden_states):
    if hidden_states is None or hidden_states == (None, None):
      return
    hid_a = (
      hidden_states[0] if isinstance(hidden_states[0], tuple) else (hidden_states[0],)
    )
    hid_c = (
      hidden_states[1] if isinstance(hidden_states[1], tuple) else (hidden_states[1],)
    )
  ...
  def reccurent_mini_batch_generator(self, num_mini_batches, num_epochs=8):
    ...
    for _ in range(num_epochs):
      ...
      for i in range(num_mini_batches):
        ...
        hid_a_batch = [
          saved_hidden_states.permute(2, 0, 1, 3)[last_was_done_t][first_traj:last_traj]
          .transpose(1, 0)
          .contiguous()
